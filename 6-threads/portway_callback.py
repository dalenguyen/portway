"""Post 6 — same metering callback as Post 5, carried forward unchanged.

Each post directory ships its own copy of the callback so Post 6 is runnable
without depending on `5-metering/`. The file is byte-identical to Post 5's
`portway_callback.py` — the gateway path is untouched in this post; threads
are an app-side concern that the proxy never sees.
"""
