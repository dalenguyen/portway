#!/usr/bin/env bash
# Post 5 — launch the LiteLLM proxy with custom metering callback.
#
# Differs from Post 4 by exporting PYTHONPATH so LiteLLM can resolve
# `portway_callback.portway_meter` (the success callback registered in config.yaml).
#
# Usage:
...
# NEW vs Post 4: put this directory on PYTHONPATH so LiteLLM can import
...
echo
echo "Tail with: tail -f 5-metering/logs/gateway.log"
