#!/usr/bin/env bash
# Post 5 — local Postgres container backing LiteLLM virtual keys AND portway_metering.
#
# Differs from Post 4 in one line: -v $(pwd)/pgdata:/var/lib/postgresql/data
# so rows survive `docker rm` + restart. Run `rm -rf pgdata/` to start clean.
#
# Usage:
